...
import brushIconImg from 'url:/src/app/img/ui/brush-pen.svg'; // placeholder, replace with glitter icon if available
...
import { GlitterBrush } from '../brushes/glitter-brush';

export const glitterBrushUi = (function () {
    const brushInterface = {
        image: brushIconImg,
        tooltip: 'Glitter Brush',
        sizeSlider: {
            min: 0.5,
            max: 100,
    ...
    LANGUAGE_STRINGS.subscribe(() => {
        brushInterface.tooltip = LANG('brush-glitter');
        alphaNames = [
            LANG('brush-pen-circle'),
            LANG('brush-pen-chalk'),
            LANG('brush-pen-calligraphy'),
    ...
    brushInterface.Ui = function (p) {
        ...
        function init() {
            ...
            sparkleSlider = new KlSlider({
                label: 'Sparkle Count',
                width: 225,
                height: 30,
                min: 1,
                max: 20,
                value: brush.getSparkleCount(),
                eventResMs: EVENT_RES_MS,
                onChange: (val) => {
                    brush.setSparkleCount(Math.round(val));
                },
                formatFunc: (val) => Math.round(val),
            ...
            div.append(
                ...
                BB.el({
                    content: sparkleSlider.getElement(),
                    css: {
                        marginBottom: '10px',
                    },
                }),
